...
void LSTMUnitLayer<Ftype, Btype>::Forward_cpu(const vector<Blob*>& bottom,
  ...
  const Ftype* C_prev = bottom[0]->cpu_data();
  const Ftype* X = bottom[1]->cpu_data();
  const Ftype* cont = bottom[2]->cpu_data();
  Ftype* C = top[0]->mutable_cpu_data();
  Ftype* H = top[1]->mutable_cpu_data();
  for (int n = 0; n < num; ++n) {
...
void LSTMUnitLayer<Ftype, Btype>::Backward_cpu(const vector<Blob*>& top,
  ...
  const int num = bottom[0]->shape(1);
  const int x_dim = hidden_dim_ * 4;
  const Btype* C_prev = bottom[0]->cpu_data();
  const Btype* X = bottom[1]->cpu_data();
  const Btype* cont = bottom[2]->cpu_data();
  const Btype* C = top[0]->cpu_data();
  const Btype* H = top[1]->cpu_data();
  const Btype* C_diff = top[0]->cpu_diff();
  const Btype* H_diff = top[1]->cpu_diff();
  Btype* C_prev_diff = bottom[0]->mutable_cpu_diff();
  Btype* X_diff = bottom[1]->mutable_cpu_diff();
  for (int n = 0; n < num; ++n) {
    for (int d = 0; d < hidden_dim_; ++d) {
