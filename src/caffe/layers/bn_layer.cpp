...
namespace caffe {
  ...
  void BNLayer<Ftype, Btype>::Reshape(const vector<Blob*>& bottom,
    ...
    Ftype* spatial_multipl_data = spatial_sum_multiplier_.mutable_cpu_data();
    caffe_set(spatial_sum_multiplier_.count(), Ftype(1),
        spatial_multipl_data);
    caffe_set(spatial_sum_multiplier_.count(), Ftype(0),
        spatial_sum_multiplier_.mutable_cpu_diff());
    // fill batch multiplier
    batch_sum_multiplier_.Reshape(N_, 1, 1, 1);
    Ftype* batch_multiplier_data = batch_sum_multiplier_.mutable_cpu_data();
    caffe_set(batch_sum_multiplier_.count(), Ftype(1),
        batch_multiplier_data);
    caffe_set(batch_sum_multiplier_.count(), Ftype(0),
  ...
  void BNLayer<Ftype, Btype>::LayerSetUp(const vector<Blob*>& bottom,
    ...
    if (this->blobs_.size() > 0) {
      ...
      this->blobs_[0].reset(new Blob<Ftype, Btype>(1, C_, 1, 1));
      shared_ptr<Filler<Ftype, Btype> > scale_filler(GetFiller<Ftype, Btype>(
          this->layer_param_.bn_param().scale_filler()));
      scale_filler->Fill(this->blobs_[0].get());

      // fill shift with shift_filler
      this->blobs_[1].reset(new Blob<Ftype, Btype>(1, C_, 1, 1));
      shared_ptr<Filler<Ftype, Btype> > shift_filler(GetFiller<Ftype, Btype>(
  ...
  void BNLayer<Ftype, Btype>::Forward_cpu(const vector<Blob*>& bottom,
      const vector<Blob*>& top) {
    const Ftype* bottom_data = bottom[0]->cpu_data();
    Dtype* top_data = top[0]->mutable_cpu_data();
    const Ftype* const_top_data = top[0]->cpu_data();

    const Ftype* scale_data = this->blobs_[0]->cpu_data();
    const Ftype* shift_data = this->blobs_[1]->cpu_data();
    ...
    case BNParameter_BNMode_LEARN:
      ...
      // EX across spatial
      caffe_cpu_gemv<Ftype>(CblasNoTrans, N_ * C_, H_ * W_,
          Ftype(1. / (H_ * W_)), bottom_data,
          spatial_sum_multiplier_.cpu_data(), Ftype(0),
          spatial_mean_.mutable_cpu_data());
      // EX across batch
      caffe_cpu_gemv<Ftype>(CblasTrans, N_, C_, Dtype(1. / N_),
          spatial_mean_.cpu_data(),
          batch_sum_multiplier_.cpu_data(), Ftype(0),
          batch_mean_.mutable_cpu_data());

      // E(X^2) across spatial
      caffe_cpu_gemv<Ftype>(CblasNoTrans, N_ * C_, H_ * W_,
          Ftype(1. / (H_ * W_)), buffer_blob_.cpu_data(),
          spatial_sum_multiplier_.cpu_data(), Ftype(0),
      ...
      caffe_powx(batch_mean_.count(), batch_mean_.cpu_data(), Ftype(2),
      ...
      // subtract mean
      caffe_cpu_gemm<Ftype>(CblasNoTrans, CblasNoTrans, N_,
          C_, 1, Ftype(1),
          batch_sum_multiplier_.cpu_data(),
          batch_mean_.cpu_data(), Ftype(0),
          spatial_mean_.mutable_cpu_data());

      caffe_cpu_gemm<Ftype>(CblasNoTrans, CblasNoTrans, N_ * C_,
          H_ * W_, 1, Ftype(-1),
          spatial_mean_.cpu_data(),
      ...
      caffe_cpu_gemm<Ftype>(CblasNoTrans, CblasNoTrans, N_,
          C_, 1, Ftype(1),
          batch_sum_multiplier_.cpu_data(),
          batch_variance_.cpu_data(), Ftype(0),
          spatial_variance_.mutable_cpu_data());
      caffe_cpu_gemm<Ftype>(CblasNoTrans, CblasNoTrans,
          N_ * C_, H_ * W_, 1, Ftype(1),
          spatial_variance_.cpu_data(),
          spatial_sum_multiplier_.cpu_data(), Ftype(0),
          buffer_blob_.mutable_cpu_data());
      ...
      // scale
      caffe_cpu_gemm<Ftype>(CblasNoTrans, CblasNoTrans, N_, C_, 1, Ftype(1),
          batch_sum_multiplier_.cpu_data(), scale_data, Ftype(0),
          spatial_variance_.mutable_cpu_data());
      caffe_cpu_gemm<Ftype>(CblasNoTrans, CblasNoTrans, N_ * C_,
          H_ * W_, 1, Ftype(1),
          spatial_variance_.cpu_data(),
      ...
      // shift
      caffe_cpu_gemm<Ftype>(CblasNoTrans, CblasNoTrans, N_, C_, 1, Ftype(1),
          batch_sum_multiplier_.cpu_data(), shift_data, Ftype(0),
          spatial_mean_.mutable_cpu_data());
      caffe_cpu_gemm<Ftype>(CblasNoTrans, CblasNoTrans,
          N_ * C_, H_ * W_, 1, Ftype(1),
          spatial_mean_.cpu_data(),
  ...
  void BNLayer<Ftype, Btype>::Backward_cpu(const vector<Blob*>& top,
    ...
    case BNParameter_BNMode_LEARN:
      ...
      // EX across spatial
      caffe_cpu_gemv<Btype>(CblasNoTrans, N_ * C_,
          H_ * W_, Btype(1), buffer_blob_.cpu_data(),
          spatial_sum_multiplier_.cpu_data(), Btype(0),
          spatial_variance_.mutable_cpu_diff());
      // EX across batch
      caffe_cpu_gemv<Btype>(CblasTrans, N_, C_, Btype(1),
          spatial_variance_.cpu_diff(),
          batch_sum_multiplier_.cpu_data(), Btype(0), scale_diff);

      // gradient w.r.t. shift
      // EX across spatial
      caffe_cpu_gemv<Btype>(CblasNoTrans, N_ * C_,
          H_ * W_, Btype(1), top_diff,
          spatial_sum_multiplier_.cpu_data(),
          Btype(0), spatial_mean_.mutable_cpu_diff());
      // EX across batch
      caffe_cpu_gemv<Btype>(CblasTrans, N_, C_,
          Btype(1), spatial_mean_.cpu_diff(),
          batch_sum_multiplier_.cpu_data(),
          Btype(0), shift_diff);
      ...
      caffe_cpu_gemm<Btype>(CblasNoTrans, CblasNoTrans, N_, C_, 1, Dtype(1),
          batch_sum_multiplier_.cpu_data(), scale_data, Dtype(0),
          spatial_variance_.mutable_cpu_data());
      caffe_cpu_gemm<Btype>(CblasNoTrans, CblasNoTrans, N_ * C_,
          H_ * W_, 1, Btype(1),
          spatial_variance_.cpu_data(),
          spatial_sum_multiplier_.cpu_data(), Btype(0),
          buffer_blob_.mutable_cpu_data());
      ...
      // EX across spatial
      caffe_cpu_gemv<Btype>(CblasNoTrans, N_ * C_, H_ * W_,
          Btype(1), bottom_diff,
          spatial_sum_multiplier_.cpu_data(), Dtype(0),
          spatial_mean_.mutable_cpu_data());
      // EX across batch
      ...
      caffe_cpu_gemm<Btype>(CblasNoTrans, CblasNoTrans,
          N_, C_, 1, Btype(1),
          batch_sum_multiplier_.cpu_data(),
          batch_mean_.cpu_data(), Btype(0),
          spatial_mean_.mutable_cpu_data());
      caffe_cpu_gemm<Btype>(CblasNoTrans, CblasNoTrans, N_ * C_,
          H_ * W_, 1, Btype(1),
          spatial_mean_.cpu_data(),
      ...
      caffe_cpu_gemv<Btype>(CblasTrans, N_, C_, Btype(1),
          spatial_mean_.cpu_data(),
          batch_sum_multiplier_.cpu_data(), Btype(0),
          batch_mean_.mutable_cpu_data());
      ...
      caffe_cpu_gemm<Btype>(CblasNoTrans, CblasNoTrans,
          ...
          spatial_mean_.mutable_cpu_data());
      caffe_cpu_gemm<Btype>(CblasNoTrans, CblasNoTrans,
          N_ * C_, H_ * W_, 1, Btype(1),
          spatial_mean_.cpu_data(),
          spatial_sum_multiplier_.cpu_data(), Btype(1), bottom_diff);

      caffe_cpu_axpby(buffer_blob_.count(), Btype(1),
          buffer_blob_.cpu_data(), Btype(-1. / (N_ * H_ * W_)),
          bottom_diff);

      // put the squares of bottom into buffer_blob_
      caffe_powx(buffer_blob_.count(), bottom_data, Btype(2),
          buffer_blob_.mutable_cpu_data());

      caffe_cpu_gemm<Btype>(CblasNoTrans, CblasNoTrans,
          N_, C_, 1, Btype(1),
          batch_sum_multiplier_.cpu_data(),
          batch_variance_.cpu_data(), Btype(0),
          spatial_variance_.mutable_cpu_data());
      caffe_cpu_gemm<Btype>(CblasNoTrans, CblasNoTrans,
          N_ * C_, H_ * W_, 1, Btype(1),
          spatial_variance_.cpu_data(),
    ...
    case BNParameter_BNMode_INFERENCE:
      ...
      // Propagate down
      // put scale * top_diff to buffer_blob_
      caffe_cpu_gemm<Btype>(CblasNoTrans, CblasNoTrans, N_, C_, 1, Btype(1),
          batch_sum_multiplier_.cpu_data(), scale_data, Btype(0),
          spatial_variance_.mutable_cpu_data());
      caffe_cpu_gemm<Btype>(CblasNoTrans, CblasNoTrans, N_ * C_,
          H_ * W_, 1, Btype(1),
          spatial_variance_.cpu_data(),
