...
void UpsampleLayer<Ftype, Btype>::LayerSetUp(const vector<Blob*>& bottom,
...
template <typename Ftype, typename Btype>
void UpsampleLayer<Ftype, Btype>::Reshape(const vector<Blob*>& bottom,
      const vector<Blob*>& top) {
...
template <typename Ftype, typename Btype>
void UpsampleLayer<Ftype, Btype>::Forward_cpu(const vector<Blob*>& bottom,
      const vector<Blob*>& top) {
  const Ftype* bottom_data = bottom[0]->cpu_data();
  const Ftype* bottom_mask_data = bottom[1]->cpu_data();
...
template <typename Ftype typename Btype>
void UpsampleLayer<Ftype, Btype>::Backward_cpu(const vector<Blob*>& top,
      const vector<bool>& propagate_down, const vector<Blob*>& bottom) {
  if (propagate_down[0]) {
    const Btype* top_diff = top[0]->cpu_diff();
    const Btype* bottom_mask_data = bottom[1]->cpu_data();
    Btype* bottom_diff = bottom[0]->mutable_cpu_diff();
